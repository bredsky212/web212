          ...
          <div className="hidden md:flex items-baseline space-x-8">
            {navItems.map((item, index) => (
              ...
              >
                <Link
                  href={item.href}
                  className={`${path === item.href ? 'text-[#8b0000]' : "text-secondary"} relative group px-1 py-2 text-sm font-medium  hover:text-[var(--accent)] transition-colors`}
                >
                  {t(item.nameKey)}
                  <span className={`absolute ${path === item.href ? "scale-x-100 " : ""} left-0 bottom-0 w-full h-0.5 bg-[var(--accent)] scale-x-0 group-hover:scale-x-100 transition-transform duration-300 ease-out origin-left`} />
                </Link>
              </motion.div>
            ))}
          </div>
              ...
              <div className="px-4 py-4 space-y-2 w-full">
                {navItems.map((item) => (
                  <Link
                    ...
                    className={` ${path === item.href ? ' text-[#8b0000] ' : ' text-[var(--text-secondary)]'} block px-4 py-3  hover:text-[var(--accent)] hover:bg-[var(--accent)]/10 rounded transition-colors`}
                  >
                    {t(item.nameKey)}
                  </Link>
                ))}
              </div>
                <div className="ml-4 w-fit border-[var(--border)]">
                  <LanguageSwitcher />
                </div>
            </div>
          </motion.div>
